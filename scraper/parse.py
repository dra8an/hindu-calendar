#!/usr/bin/env python3
"""Phase 2: Parse saved drikpanchang.com HTML into a CSV.

Extracts tithi (1-30) for each day from month panchang HTML pages.

Usage:
    python3 parse.py                       # Parse all available months
    python3 parse.py --start-year 2024 --end-year 2025
    python3 parse.py --output /path/to/output.csv
"""
...
from config import PARSED_CSV, RAW_MONTH_DIR, tithi_to_number
...
def build_csv(start_year, end_year, output_path):
    ...
    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            html_path = os.path.join(RAW_MONTH_DIR, f"{year:04d}-{month:02d}.html")
...
def main():
    parser = argparse.ArgumentParser(description="Parse drikpanchang HTML → CSV")
