/* Quick check of 2 remaining failing dates */
#include <stdio.h>
#include <math.h>
#include "moshier.h"
...
static void check_date(int y, int m, int d, int exp_tithi) {
    ...
    double our_in_tithi = fmod(our_elong, 12.0);
    double se_in_tithi = fmod(se_elong, 12.0);

    /* Moon and sun differences at our sunrise */
    ...
    /* Elongation difference at our sunrise */
    double elong_diff = (our_elong - se_elong) * 3600.0;
...
int main(void) {
    ...
    printf("# (in_tithi = distance from start of tithi in arcsec; 0 or 43200 = boundary)\n\n");
    check_date(1965, 5, 30, 30);
    check_date(2001, 9, 20,  4);
    swe_close();
    return 0;
}
