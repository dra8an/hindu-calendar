...
class Sun {
    ...
    private var ssTbl = [[Double]](repeating: [Double](repeating: 0.0, count: 24), count: 9)
    private var ccTbl = [[Double]](repeating: [Double](repeating: 0.0, count: 24), count: 9)
    ...
    private func vsop87EarthLongitude(_ jdTt: Double) -> Double {
        let T = (jdTt - Sun.J2000_JD) / Sun.TIMESCALE

        for i in 0..<9 {
            if Sun.EAR_MAX_HARMONIC[i] > 0 {
                let sr = (Sun.mods3600(Sun.FREQS[i] * T) + Sun.PHASES[i]) * Sun.STR
                sscc(i, sr, Sun.EAR_MAX_HARMONIC[i])
            }
        }
        ...
        while true {
            let np = Int(Sun.EARARGS[pIdx]); pIdx += 1
            if np < 0 { break }

            if np == 0 {
                let nt = Int(Sun.EARARGS[pIdx]); pIdx += 1
                var cu = Sun.EARTABL[plIdx]; plIdx += 1
                for _ in 0..<nt {
                    cu = cu * T + Sun.EARTABL[plIdx]; plIdx += 1
                }
                sl += Sun.mods3600(cu)
                continue
            }
            ...
            var cv = 0.0, sv = 0.0
            for _ in 0..<np {
                let j = Int(Sun.EARARGS[pIdx]); pIdx += 1
                let m = Int(Sun.EARARGS[pIdx]) - 1; pIdx += 1
                if j != 0 {
                    var k = j < 0 ? -j : j
                    k -= 1
                    var su = ssTbl[m][k]
            ...
            let nt = Int(Sun.EARARGS[pIdx]); pIdx += 1
            var cu = Sun.EARTABL[plIdx]; plIdx += 1
            var su = Sun.EARTABL[plIdx]; plIdx += 1
            for _ in 0..<nt {
                cu = cu * T + Sun.EARTABL[plIdx]; plIdx += 1
                su = su * T + Sun.EARTABL[plIdx]; plIdx += 1
            }
            sl += cu * cv + su * sv
        }
    ...
    private func embEarthCorrection(_ jdTt: Double, _ L_emb_rad: Double) -> Double {
        let T = (jdTt - Sun.J1900_JD) / 36525.0
        ...
        if L < 0 { L += 360.0 }
        let L_moon_rad = L * Sun.DEG2RAD

        return -rMoon * cos(B) * sin(L_moon_rad - L_emb_rad) / (Sun.EARTH_MOON_MRAT + 1.0)
    }

    func deltaT(_ jdUt: Double) -> Double {
        return deltaTSeconds(jdUt) / 86400.0
    }

    private func deltaTSeconds(_ jdUt: Double) -> Double {
        let ymd = JulianDay.revjul(jdUt)
        let y = Double(ymd.year) + (Double(ymd.month) - 0.5) / 12.0

        if y >= Double(Sun.DT_TAB_START) && y < Double(Sun.DT_TAB_END) + 1 {
            let idx = y - Double(Sun.DT_TAB_START)
            var i = Int(idx)
            if i >= Sun.DT_TAB.count - 1 { i = Sun.DT_TAB.count - 2 }
            let frac = idx - Double(i)
            return Sun.DT_TAB[i] + frac * (Sun.DT_TAB[i + 1] - Sun.DT_TAB[i])
        } else if y < Double(Sun.DT_TAB_START) {
    ...
    private func jdUtToTt(_ jdUt: Double) -> Double {
        return jdUt + deltaTSeconds(jdUt) / 86400.0
    }

    func nutation(_ jdTt: Double) -> (dpsi: Double, deps: Double) {
        ...
        for i in 0..<13 {
            let arg = Double(Sun.NT_ARGS[i][0]) * D + Double(Sun.NT_ARGS[i][1]) * M
                + Double(Sun.NT_ARGS[i][2]) * Mp + Double(Sun.NT_ARGS[i][3]) * F
                + Double(Sun.NT_ARGS[i][4]) * Om
            sumDpsi += (Sun.NT_S0[i] + Sun.NT_S1[i] * T) * sin(arg)
            sumDeps += (Sun.NT_C0[i] + Sun.NT_C1[i] * T) * cos(arg)
        }

        return (sumDpsi * 0.0001 / 3600.0, sumDeps * 0.0001 / 3600.0)
    }

    func meanObliquity(_ jdTt: Double) -> Double {
        let T = (jdTt - 2451545.0) / 36525.0
        let U = T / 100.0
    ...
    private func solarPosition(_ jdUt: Double) -> (longitude: Double, declination: Double, nutLon: Double) {
        let jdTt = jdUtToTt(jdUt)
        let T = (jdTt - Sun.J2000_JD) / 36525.0

        let sl = vsop87EarthLongitude(jdTt)
        let L_emb_j2000 = sl * Sun.STR

        let pA = (5029.0966 + 1.11113 * T - 0.000006 * T * T) * T
        let L_emb_date = L_emb_j2000 + pA * Sun.STR
        ...
        var L_apparent = L_sun_date + nut.dpsi * Sun.DEG2RAD

        L_apparent -= 20.496 * Sun.STR

        let apparent = Sun.normalizeDeg(L_apparent * Sun.RAD2DEG)

        let eps0 = meanObliquity(jdTt)
        let eps = (eps0 + nut.deps) * Sun.DEG2RAD
        let lam = apparent * Sun.DEG2RAD
        let declination = asin(sin(eps) * sin(lam)) * Sun.RAD2DEG

        return (apparent, declination, nut.dpsi)
    }

    func solarLongitude(_ jdUt: Double) -> Double {
        return solarPosition(jdUt).longitude
    }

    func solarDeclination(_ jdUt: Double) -> Double {
        return solarPosition(jdUt).declination
    }

    func solarRa(_ jdUt: Double) -> Double {
        let jdTt = jdUtToTt(jdUt)
        let nut = nutation(jdTt)
        let eps0 = meanObliquity(jdTt)
        let eps = (eps0 + nut.deps) * Sun.DEG2RAD
        let lam = solarLongitude(jdUt) * Sun.DEG2RAD
        let ra = atan2(cos(eps) * sin(lam), cos(lam)) * Sun.RAD2DEG
        return Sun.normalizeDeg(ra)
    }

    func nutationLongitude(_ jdUt: Double) -> Double {
        let jdTt = jdUtToTt(jdUt)
        return nutation(jdTt).dpsi
    }

    func meanObliquityUt(_ jdUt: Double) -> Double {
        return meanObliquity(jdUtToTt(jdUt))
    }

    func trueObliquity(_ jdUt: Double) -> Double {
        let jdTt = jdUtToTt(jdUt)
