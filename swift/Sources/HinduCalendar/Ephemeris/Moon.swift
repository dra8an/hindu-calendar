...
class Moon {
    ...
    private var ss = [[Double]](repeating: [Double](repeating: 0.0, count: 8), count: 5)
    private var cc = [[Double]](repeating: [Double](repeating: 0.0, count: 8), count: 5)

    private var SWELP = 0.0, M_sun = 0.0, MP = 0.0, D = 0.0, NF = 0.0
    private var T = 0.0, T2 = 0.0
    private var Ve = 0.0, Ea = 0.0, Ma = 0.0, Ju = 0.0, Sa = 0.0
    ...
    private func sscc(_ k: Int, _ arg: Double, _ n: Int) {
    ...
    private func chewm(_ pt: [Int16], _ nlines: Int, _ nangles: Int, _ typflg: Int) -> Double {
        var ans = 0.0
        var idx = 0
        for _ in 0..<nlines {
            var k1 = 0
            ...
            for _ in 0..<nangles {
                ...
                if j != 0 {
                    var k = j < 0 ? -j : j
                    k -= 1
                    // m is implicit from the loop index in the angle ordering (D, M_sun, MP, NF)
                    // But actually we use the column index m directly
                }
            }
            // Reset and redo properly - the Java code uses m as the loop index
            idx -= nangles
            k1 = 0
            sv = 0.0; cv = 0.0
            for m in 0..<nangles {
    ...
    // ===== Mean elements =====
    private func meanElements() {
        let fracT = T.truncatingRemainder(dividingBy: 1.0)

        M_sun = Moon.mods3600(129600000.0 * fracT - 3418.961646 * T + 1287104.76154)
        M_sun += ((((((((
            1.62e-20 * T
                - 1.0390e-17) * T
                - 3.83508e-15) * T
        ...
        NF = Moon.mods3600(1739232000.0 * fracT + 295263.0983 * T
            - 2.079419901760e-01 * T + 335779.55755)

        MP = Moon.mods3600(1717200000.0 * fracT + 715923.4728 * T
            - 2.035946368532e-01 * T + 485868.28096)

        D = Moon.mods3600(1601856000.0 * fracT + 1105601.4603 * T
            + 3.962893294503e-01 * T + 1072260.73512)

        SWELP = Moon.mods3600(1731456000.0 * fracT + 1108372.83264 * T
            - 6.784914260953e-01 * T + 785939.95571)

        NF += ((Moon.Z[2] * T + Moon.Z[1]) * T + Moon.Z[0]) * T2
        MP += ((Moon.Z[5] * T + Moon.Z[4]) * T + Moon.Z[3]) * T2
        D += ((Moon.Z[8] * T + Moon.Z[7]) * T + Moon.Z[6]) * T2
        SWELP += ((Moon.Z[11] * T + Moon.Z[10]) * T + Moon.Z[9]) * T2
    }

    private func meanElementsPl() {
        Ve = Moon.mods3600(210664136.4335482 * T + 655127.283046)
        Ve += ((((((((
            -9.36e-023 * T
                - 1.95e-20) * T
                + 6.097e-18) * T
        ...
        Ea = Moon.mods3600(129597742.26669231 * T + 361679.214649)
        Ea += ((((((((-1.16e-22 * T
            + 2.976e-19) * T
            + 2.8460e-17) * T
            - 1.08402e-14) * T
        ...
        Ma = Moon.mods3600(68905077.59284 * T + 1279559.78866)
        Ma += (-1.043e-5 * T + 9.38012e-3) * T2

        Ju = Moon.mods3600(10925660.428608 * T + 123665.342120)
        Ju += (1.543273e-5 * T - 3.06037836351e-1) * T2

        Sa = Moon.mods3600(4399609.65932 * T + 180278.89694)
        Sa += ((4.475946e-8 * T - 6.874806E-5) * T + 7.56161437443E-1) * T2
    }

    private func lunarPerturbations() -> Double {
        ...
        sscc(0, Moon.STR * D, 6)
        sscc(1, Moon.STR * M_sun, 4)
        sscc(2, Moon.STR * MP, 4)
        sscc(3, Moon.STR * NF, 4)

        moonpol0 = chewm(Moon.LRT2, Moon.NLRT2, 4, 2)

        f_ve = 18.0 * Ve - 16.0 * Ea

        var g_arg = Moon.STR * (f_ve - MP)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc = 6.367278 * cg + 12.747036 * sg
        l1 = 23123.70 * cg - 10570.02 * sg
        l2 = Moon.Z[12] * cg + Moon.Z[13] * sg

        g_arg = Moon.STR * (10.0 * Ve - 3.0 * Ea - MP)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += -0.253102 * cg + 0.503359 * sg
        l1 += 1258.46 * cg + 707.29 * sg
        l2 += Moon.Z[14] * cg + Moon.Z[15] * sg

        g_arg = Moon.STR * (8.0 * Ve - 13.0 * Ea)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += -0.187231 * cg - 0.127481 * sg
        l1 += -319.87 * cg - 18.34 * sg
        l2 += Moon.Z[16] * cg + Moon.Z[17] * sg

        let a = 4.0 * Ea - 8.0 * Ma + 3.0 * Ju
        g_arg = Moon.STR * a
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += -0.866287 * cg + 0.248192 * sg
        l1 += 41.87 * cg + 1053.97 * sg
        l2 += Moon.Z[18] * cg + Moon.Z[19] * sg
        ...
        l2 += Moon.Z[20] * cg + Moon.Z[21] * sg

        g_arg = Moon.STR * (f_ve - 2.0 * MP)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += 0.352185 * cg + 0.705041 * sg
        l1 += 1283.59 * cg - 586.43 * sg

        g_arg = Moon.STR * (2.0 * Ju - 5.0 * Sa)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += -0.034700 * cg + 0.160041 * sg
        l2 += Moon.Z[22] * cg + Moon.Z[23] * sg

        g_arg = Moon.STR * (SWELP - NF)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += 0.000116 * cg + 7.063040 * sg
        l1 += 298.8 * sg

        l3 = Moon.Z[24] * sin(Moon.STR * M_sun)
        l4 = 0

        l2 += moonpol0

        moonpol0 = chewm(Moon.LRT, Moon.NLRT, 4, 1)

        g_arg = Moon.STR * (2.0 * Ve - 3.0 * Ea)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += -0.343550 * cg - 0.000276 * sg
        l1 += 105.90 * cg + 336.53 * sg

        g_arg = Moon.STR * (f_ve - 2.0 * D)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += 0.074668 * cg + 0.149501 * sg
        l1 += 271.77 * cg - 124.20 * sg

        g_arg = Moon.STR * (f_ve - 2.0 * D - MP)
        cg = cos(g_arg); sg = sin(g_arg)
        ...
        g_arg = Moon.STR * (3.0 * Ve - 4.0 * Ea)
        cg = cos(g_arg); sg = sin(g_arg)
        l_acc += -0.175074 * cg + 0.003035 * sg
        l1 += 49.17 * cg + 150.57 * sg

        g_arg = Moon.STR * (2.0 * (Ea + D - MP) - 3.0 * Ju + 213534.0)
        l1 += 158.4 * sin(g_arg)

        l1 += moonpol0

        g_arg = Moon.STR * (2.0 * (Ea - Ju + D) - MP + 648431.172)
        l_acc += 1.14307 * sin(g_arg)

        g_arg = Moon.STR * (Ve - Ea + 648035.568)
        l_acc += 0.82155 * sin(g_arg)

        g_arg = Moon.STR * (3.0 * (Ve - Ea) + 2.0 * D - MP + 647933.184)
        l_acc += 0.64371 * sin(g_arg)

        g_arg = Moon.STR * (Ea - Ju + 4424.04)
        l_acc += 0.63880 * sin(g_arg)

        g_arg = Moon.STR * (SWELP + MP - NF + 4.68)
        l_acc += 0.49331 * sin(g_arg)

        g_arg = Moon.STR * (SWELP - MP - NF + 4.68)
        l_acc += 0.4914 * sin(g_arg)

        g_arg = Moon.STR * (SWELP + NF + 2.52)
        l_acc += 0.36061 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * Ve - 2.0 * Ea + 736.2)
        l_acc += 0.30154 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * Ea - 3.0 * Ju + 2.0 * D - 2.0 * MP + 36138.2)
        l_acc += 0.28282 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * Ea - 2.0 * Ju + 2.0 * D - 2.0 * MP + 311.0)
        l_acc += 0.24516 * sin(g_arg)

        g_arg = Moon.STR * (Ea - Ju - 2.0 * D + MP + 6275.88)
        l_acc += 0.21117 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * (Ea - Ma) - 846.36)
        l_acc += 0.19444 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * (Ea - Ju) + 1569.96)
        l_acc -= 0.18457 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * (Ea - Ju) - MP - 55.8)
        l_acc += 0.18256 * sin(g_arg)

        g_arg = Moon.STR * (Ea - Ju - 2.0 * D + 6490.08)
        l_acc += 0.16499 * sin(g_arg)

        g_arg = Moon.STR * (Ea - 2.0 * Ju - 212378.4)
        l_acc += 0.16427 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * (Ve - Ea - D) + MP + 1122.48)
        l_acc += 0.16088 * sin(g_arg)

        g_arg = Moon.STR * (Ve - Ea - MP + 32.04)
        l_acc -= 0.15350 * sin(g_arg)

        g_arg = Moon.STR * (Ea - Ju - MP + 4488.88)
        l_acc += 0.14346 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * (Ve - Ea + D) - MP - 8.64)
        l_acc += 0.13594 * sin(g_arg)

        g_arg = Moon.STR * (2.0 * (Ve - Ea - D) + 1319.76)
        l_acc += 0.13432 * sin(g_arg)

        g_arg = Moon.STR * (Ve - Ea - 2.0 * D + MP - 56.16)
        l_acc -= 0.13122 * sin(g_arg)

        g_arg = Moon.STR * (Ve - Ea + MP + 54.36)
        l_acc -= 0.12722 * sin(g_arg)

        g_arg = Moon.STR * (3.0 * (Ve - Ea) - MP + 433.8)
        l_acc += 0.12539 * sin(g_arg)

        g_arg = Moon.STR * (Ea - Ju + MP + 4002.12)
        l_acc += 0.10994 * sin(g_arg)

        g_arg = Moon.STR * (20.0 * Ve - 21.0 * Ea - 2.0 * D + MP - 317511.72)
        l_acc += 0.10652 * sin(g_arg)

        g_arg = Moon.STR * (26.0 * Ve - 29.0 * Ea - MP + 270002.52)
        l_acc += 0.10490 * sin(g_arg)

        g_arg = Moon.STR * (3.0 * Ve - 4.0 * Ea + D - MP - 322765.56)
        l_acc += 0.10386 * sin(g_arg)

        moonpol0 = chewm(Moon.LR, Moon.NLR, 4, 1)
        l_acc += (((l4 * T + l3) * T + l2) * T + l1) * T * 1.0e-5
        moonpol0 = SWELP + l_acc + 1.0e-4 * moonpol0

        return Moon.STR * Moon.mods3600(moonpol0)
    }

    func lunarLongitude(_ jdUt: Double) -> Double {
        let jdTt = jdUt + sun.deltaT(jdUt)
        T = (jdTt - Moon.J2000) / 36525.0
        T2 = T * T

        meanElements()
        meanElementsPl()
        let lonRad = lunarPerturbations()

        var lonDeg = lonRad * (180.0 / Double.pi)

        // Distance-dependent light-time correction
        do {
            let cosL = cos(Moon.STR * MP)
            let cos2dL = cos(Moon.STR * (2.0 * D - MP))
            let cos2d = cos(Moon.STR * (2.0 * D))
            let cos2l = cos(Moon.STR * (2.0 * MP))
            let cosLp = cos(Moon.STR * M_sun)
